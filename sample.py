...
def roundrobininsert(ratingstablename, userid, itemid, rating, openconnection):
    ...
    cur = con.cursor()
    RROBIN_TABLE_PREFIX = 'rrobin_part'
    # First, get the current count to determine which partition this new row should go to
    cur.execute("select count(*) from " + ratingstablename + ";");
    total_rows = (cur.fetchall())[0][0]
    numberofpartitions = count_partitions(RROBIN_TABLE_PREFIX, openconnection)
    # Calculate partition index before inserting (since round-robin is based on row position)
...
def rangeinsert(ratingstablename, userid, itemid, rating, openconnection):
    ...
    cur.execute("insert into " + ratingstablename + "(userid, movieid, rating) values (" + str(userid) + "," + str(itemid) + "," + str(rating) + ");")
    # Then insert into appropriate partition
